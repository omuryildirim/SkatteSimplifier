...
export async function yearly() {
  ...
  if (!year) {
    throw new Error(
      'Year is required! Please provide a year as first argument: pnpm calculate 2024',
    );
  } else if (!personNummer) {
    throw new Error(
      'Personnummer is required! Please provide a personnummer as second argument: pnpm calculate 2024 198901017777',
    );
  } else if (!name) {
    throw new Error(
      'Name is required! Please provide a name as third argument: pnpm calculate 2024 198901017777 "John Doe"',
    );
  }
  ...
  try {
    ...
    const lastYearRecords = parsePreviousYear(lastYearFile, lastYear);
